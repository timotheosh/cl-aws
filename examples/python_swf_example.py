...
service = 'swf'
region = 'us-east-1'
host = 'swf.{}.amazonaws.com'.format(region)
endpoint = 'https://swf.{}.amazonaws.com/'.format(region)
# POST requests use a content type header. For DynamoDB,
# the content is JSON.
content_type = 'application/x-amz-json-1.0'
# DynamoDB requires an x-amz-target header that has this format:
#     DynamoDB_<API version>.<operationName>
amz_target = 'SimpleWorkflowService.ListDomains'

# Request parameters for CreateTable--passed in a JSON block.
request_parameters = '{'
request_parameters += '"registrationStatus":"REGISTERED"'
request_parameters += '}'
...
if access_key is None or secret_key is None:
    ...
    config = AwsConfigMFA()
    creds = config.getTokenCredentials("dev")
